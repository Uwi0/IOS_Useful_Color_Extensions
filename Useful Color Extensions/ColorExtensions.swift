...
extension Color {
    ...
    // Returns n different colors if possible.
    // If n <= mainColorList.count return n different colors
    // Otherwise just return n random colors (thre will be some repetition)
    static func randomColorsNDifferent( n: Int) -> [Color] {
        var theRandomColors: [Color] = []
